...
-- Initializes a new scroll buffer
---@param height number|nil
---@param width number|nil
---@param vram_buffer number|nil
---@param gpu table|nil
---@return table scrollBuffer
function scrollBuffer.new(height, width, vram_buffer, gpu)
    local self = setmetatable({}, scrollBuffer)
    self.height = height or _G.height
    self.width = width or _G.width
    self.buffer_lines = {}
    self.visible_lines = {}
    ...
    self.gpu = gpu or _G.primary_gpu
    self.vram_buffer = vram_buffer or _G.vram_buffer
    self.logging = false
...
-- Sets the GPU for rendering
---@param gpu table
function scrollBuffer:setGPU(gpu)
    self.gpu = gpu
end

-- Sets the VRAM buffer for rendering
---@param vram_buffer number
function scrollBuffer:setVramBuffer(vram_buffer)
    self.vram_buffer = vram_buffer
end

-- Updates the resolution and visible buffer
---@param height number|nil
---@param width number|nil
function scrollBuffer:updateResolution(height, width)
    self.height = height or _G.height
    self.width = width or _G.width
...
function scrollBuffer:scrollDown()
    local end_index = #self.buffer_lines - self.height + 1
    if end_index < 1 then
        end_index = 1
    end
...
function scrollBuffer:scrollToBottom()
    self.buffer_index = #self.buffer_lines - self.height
    if self.buffer_index < 1 then
        self.buffer_index = 1
    end
    self:updateVisibleBuffer()
...
function scrollBuffer:highlightText(search_term, search_results, iterator)
    for i, result in ipairs(search_results) do
        ...
        if screen_line >= 1 and screen_line <= self.height - 2 then
            if i == iterator then
                draw.highlightText(search_term, screen_column, screen_line, colors.WHITE, colors.GREY)
            else
                draw.highlightText(search_term, screen_column, screen_line, colors.BLACK, colors.WHITE)
            end
        end
    end
end
...
function scrollBuffer:goToEnd()
    self.buffer_index = #self.buffer_lines - self.height + 1
    if self.buffer_index < 1 then
        self.buffer_index = 1
    end
    self:updateVisiblePager()
...
function scrollBuffer:lineDown()
    if self.buffer_index < #self.buffer_lines - self.height + 1 then
        self.buffer_index = self.buffer_index + 1
        self:updateVisiblePager()
    end
end
...
function scrollBuffer:updateVisiblePager()
    ...
    local screen_index = 1
    local end_index = self.buffer_index + height - 2
